...
class MenuCoreAdapter(object):
    ...
    def _formatNewEntry(self, custom, url, icon):
        """Return a menu-like structure with the new additional element"""
        return {'title': mf(custom['element-name']),
                'description': mf(custom['element-descr']),
                'action': url,
                'selected': False,
                'icon': icon,
                'submenu': None,
    ...
    def getMenuCustomization(self, data, results):
        raise NotImplementedError("You must provide the "
                                  "getMenuCustomization method")
    ...
    def _get_menu_customizations(self):
        ...
        try:
            view = getMultiAdapter((context, context.REQUEST),
                                   name=u'customize-factoriesmenu')
...
class FolderFactoryMenuAdapter(PloneSiteFactoryMenuAdapter):
    ...
    def getMenuCustomization(self, data, results):
        ...
        try:
            viewOnPortal = getMultiAdapter(
                (portal, context.REQUEST), name=u'customize-factoriesmenu')
        except ComponentLookupError:
            return results
        view = getMultiAdapter((context, context.REQUEST),
                               name=u'customize-factoriesmenu')
        if viewOnPortal.inherit and view.inherit:
            siteResults = ICustomFactoryMenuProvider(
                portal).getMenuCustomization(data, results)
            results = PloneSiteFactoryMenuAdapter.getMenuCustomization(
                self, data, siteResults)
        else:
            results = PloneSiteFactoryMenuAdapter.getMenuCustomization(
                self, data, results)
        return results
